...
class ResidualAttnDropBlockBlock(nn.Module):
    def __init__(self, cin, cout, stride=1, attn="cbam", bn=True, act="silu",
                 use_deform=False, dropblock_p=0.0, dropblock_size=7):
        super().__init__()
        self.conv1 = ConvBNAct(cin, cout, k=3, s=stride, p=1, bn=bn, act=act)
        Conv2 = DeformConvBNAct if use_deform else ConvBNAct
        self.conv2 = Conv2(cout, cout, k=3, s=1, p=1, bn=bn, act=None)
...
class ResAttnDeformDropBackbone(nn.Module):
    def __init__(self, in_ch=3, base_ch=64, layers=(2, 2, 2, 2),
                 attn="cbam", bn=True, act="silu",
                 deform_stages=(False, False, True, True),
                 dropblock_p=0.0, dropblock_size=7):
        super().__init__()
        ...
        self.stage2 = self._make_stage(base_ch, ch2, layers[0], first_stride=2,
                                       attn=attn, bn=bn, act=act,
                                       use_deform=deform_stages[0],
                                       dropblock_p=dropblock_p, dropblock_size=dropblock_size)
        self.stage3 = self._make_stage(ch2, ch3, layers[1], first_stride=2,
                                       attn=attn, bn=bn, act=act,
                                       use_deform=deform_stages[1],
                                       dropblock_p=dropblock_p, dropblock_size=dropblock_size)
        self.stage4 = self._make_stage(ch3, ch4, layers[2], first_stride=2,
                                       attn=attn, bn=bn, act=act,
                                       use_deform=deform_stages[2],
                                       dropblock_p=dropblock_p, dropblock_size=dropblock_size)
        self.stage5 = self._make_stage(ch4, ch5, layers[3], first_stride=2,
                                       attn=attn, bn=bn, act=act,
                                       use_deform=deform_stages[3],
                                       dropblock_p=dropblock_p, dropblock_size=dropblock_size)

    def _make_stage(self, cin, cout, n, first_stride, attn, bn, act, use_deform, dropblock_p, dropblock_size):
        blocks = []
        blocks.append(
            ResidualAttnDropBlockBlock(
                cin, cout, stride=first_stride, attn=attn, bn=bn, act=act,
                use_deform=use_deform, dropblock_p=dropblock_p, dropblock_size=dropblock_size
            )
        )
        for _ in range(n - 1):
            blocks.append(
                ResidualAttnDropBlockBlock(
                    cout, cout, stride=1, attn=attn, bn=bn, act=act,
                    use_deform=use_deform, dropblock_p=dropblock_p, dropblock_size=dropblock_size
                )
            )
...
class FullModel(nn.Module):
    def __init__(self,
                 ...
                 fpn_use_act=False):
        super().__init__()
        self.backbone = ResAttnDeformDropBackbone(
            in_ch=in_ch,
            base_ch=base_ch,
            layers=layers,
            attn=attn,
