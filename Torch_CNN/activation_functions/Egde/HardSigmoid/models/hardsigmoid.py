...
class SEGate(nn.Module):
    ...
    def forward(self, x):
        ...
        w = F.hardsigmoid(self.fc2(w))   # <-- HARD-SIGMOID
        return x * w
...
class BottleneckSE(nn.Module):
    def __init__(
        ...
        self.out_act = nn.SiLU(inplace=True) if act == "silu" else nn.ReLU(inplace=True)
...
def make_stage(cin, cout, num_blocks, stride, drop_path_rate, act="silu"):
    blocks = []
...
class MidResNetSE(nn.Module):
    def __init__(self, num_classes=10, drop_path_rate=0.10, act="silu"):
        super().__init__()
...
def build_model(num_classes: int = 10) -> nn.Module:
    return MidResNetSE(num_classes=num_classes, drop_path_rate=0.10, act="silu")