...
def _get_act(act: str) -> nn.Module:
    ...
    raise ValueError("act must be 'relu' or 'silu' (or 'swish')")
...
class ConvBNAct(nn.Module):
    def __init__(self, cin, cout, k=3, s=1, p=None, groups=1, act="silu"):
        super().__init__()
        if p is None:
            p = k // 2
...
class SEGate(nn.Module):
    """SE-style channel attention gate (sigmoid)."""
    def __init__(self, channels: int, reduction: int = 16, act: str = "silu"):
        super().__init__()
...
class MidResNetSE(nn.Module):
    def __init__(self, num_classes=10, act="silu", drop_path_rate=0.10):
        super().__init__()
...
def build_model(num_classes: int = 10, act: str = "silu") -> nn.Module:
    return MidResNetSE(num_classes=num_classes, act=act, drop_path_rate=0.10)

if __name__ == "__main__":
    x = torch.randn(2, 3, 224, 224)
    m_relu = build_model(10, act="relu")
    m_silu = build_model(10, act="silu")
    print("relu out:", m_relu(x).shape)
    print("silu out:", m_silu(x).shape)

## from models.act_select import build_model

# model_relu = build_model(num_classes=10, act="relu")
# model_silu = build_model(num_classes=10, act="silu")